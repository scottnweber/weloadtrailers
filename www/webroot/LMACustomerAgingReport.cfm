...
<cfoutput>
	...
	<div class="white-con-area" style="height: 36px;background-color: ##82bbef;">
		<div style="float: left;"><h2 style="color:white;font-weight:bold;margin-left: 12px;">Customer Aging Report</h2></div>
	</div>
			...
			<cfform name="frmCommissionReport" action="##" method="post">
				<div class="form-con">
					<fieldset>
	            		...
	            		<div style="width:110px; float:left;">&nbsp;</div>
	           			<div style="float:left;">
			                <cfinput type="radio" name="groupBy" value="CustomerName" id="CustomerName"  style="width:15px; padding:0px 0px 0 0px;margin-bottom:1px;" checked/>
			                <label class="normal" for="CustomerName" style="text-align:left; padding:0 0 0 0;width: 100px;" >Customer Name</label>
			            </div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('##sReport').click(function(){	
			 	if(checkdate()){
					var url = "../reports/LMACustomerAgingReport.cfm?dsn=#dsn#";
					url += "&dateFrom="+$("##dateFrom"). val();
					url += "&dateTo="+$("##dateTo"). val();

					<cfif request.qryCustomersList.recordcount>
