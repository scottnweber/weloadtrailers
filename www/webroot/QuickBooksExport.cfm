...
<cfif structKeyExists(form, "ExportInvoices")>
    ...
    <cflocation url="index.cfm?event=QuickBooksExport&#session.URLToken#">
</cfif>
<cfinvoke component="#variables.objloadGateway#" method="getSystemSetupOptions" returnvariable="request.qGetSystemSetupOptions" />
<cfinvoke component="#variables.objLoadGatewayNew#" method="getQBARLoads" frmStruct="#form#" returnvariable="qLoads" />
<cfinvoke component="#variables.objLoadGatewayNew#" method="getQBARCustomerCarrierTotals" frmStruct="#form#" returnvariable="qTotals" />
...
<cfoutput>
    ...
    <script>
        $(document).ready(function(){
            ...
            <cfif NOT (ListContains(session.rightsList,'modifySystemSetup',',') AND NOT ListContains(session.rightsList,'HideCustomerPricing',','))>
                ...
                var lastSel = $("##InvoiceStatus option:selected");
                ...
                $("##InvoiceStatus").click(function(){
                    alert('Only users with rights to the Settings menu can change the Load Status Update.');
                    lastSel = $("##InvoiceStatus option:selected");
                });
            </cfif>
            ...
            $('##InvoiceStatus').change(function(){
                ...
                $.ajax({
                    ...
                    url: path,
                    dataType: "json",
                    data: {fieldName:'QBInvoiceStatusUpdateTo',value:val,CompanyID:'#session.CompanyID#'},
                    success: function(data){
                    }
                })
            })
        ...
        <div class="form-search" style="width:100%;background-color: #request.qGetSystemSetupOptions.BackgroundColorForContentAccounting# !important;padding-bottom: 10px;">
            <cfform id="ARExportForm" name="ARExportForm" action="" method="post" preserveData="yes">
                ...
                <fieldset style="width:45%;padding:5px;float:left;">
                    ...
                    <div style="float: left;">
                        <label style="font-weight: bold;width:75px;text-align: left;margin-left: 5px;color:##2f9d01">Invoice Status<br>Update To:</label>
                        <select style="color:##2f9d01" id="InvoiceStatus" name="InvoiceStatus" class="statusUpdate">
                            <cfif len(trim(request.qGetSystemSetupOptions.QBInvoiceStatusUpdateTo))>
                                <cfset statusUpdateTo = request.qGetSystemSetupOptions.QBInvoiceStatusUpdateTo>
                            <cfelse>
                                <cfset statusUpdateTo = request.qGetSystemSetupOptions.ARAndAPExportStatusID>
                            </cfif>
                            <cfset showStatus = 0>
                            <cfloop query="request.qLoadStatus">
                                <cfif request.qLoadStatus.value EQ request.qGetSystemSetupOptions.ARAndAPExportStatusID>
                                    <cfset showStatus = 1>
                                </cfif>
    ...
    <cfif structKeyExists(session, "ExportRespMsg")>
        <div id="message" class="msg-area-#session.ExportRespMsg.res#">#session.ExportRespMsg.msg#</div>
    ...
    <table width="914" border="0" class="data-table" cellspacing="0" cellpadding="0" style="color:##000000;">
        ...
        <tbody id="data-content">
            ...
            <cfelse>
                <tr>
                    <td colspan="12" align="center" style="border-left: 1px solid ##5d8cc9;border-right: 1px solid ##5d8cc9;">No Records Found.</td>
                </tr>
            </cfif>
        </tbody>
        <tfoot>
            <tr>
                <td colspan="12" align="left" valign="middle" class="footer-bg" style="border-left: 1px solid ##5d8cc9;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;border-right: 1px solid ##5d8cc9;">
                    <div class="up-down">
