<cfcomponent>
	...
	<cffunction name="OnApplicationStart" >
		...
		<cfset Application.FMCSAdsn = "FMCSA">
		
		<cfset Application.gAPI='AIzaSyAMvv7YySXPnt110qj7509OH-9N3HgCJmw' > <!---Googple Maps API--->
    ...
    <cffunction name = "onSessionEnd" returnType = "void" output = "true">
        ...
        <cftry>
			...
			<!---Query to select  customers table--->
			<cfquery name="qryGetCustomerInuseBy" datasource="#arguments.appScope.dsn#">
				select CustomerID from Customers 
				where sessionId=<cfqueryparam value="#arguments.sessionScope.sessionid#" cfsqltype="cf_sql_varchar">
			</cfquery>
			<cfif qryGetCustomerInuseBy.recordcount>
				<!---Query to update customers table--->
				<cfquery name="qryCustomerGetInuseBy" datasource="#arguments.appScope.dsn#">
					update Customers 
					set 
						InUseBy=null,
						InUseOn=null,
			...
			<!---Query to select  carriers table--->
				<cfquery name="qryGetInuseBy" datasource="#arguments.appScope.dsn#">
			...
			<!---Query to select  employees table--->
			<cfquery name="qryGetInuseBy" datasource="#arguments.appScope.dsn#">
			...
			<!---Query to select  equipments table--->
			<cfquery name="qryGetInuseBy" datasource="#arguments.appScope.dsn#">
	...
	<cffunction name="OnError" access="public" returntype="void" output="true" hint="Fires when an exception occures that is not caught by a try/catch block">
        ...
        <cfargument name="EventName" type="string" required="false" default=""/>
       	...
       	<cfif structKeyExists(arguments.Exception, "cause") and structKeyExists(arguments.Exception.cause, "Message") and arguments.Exception.cause.Message EQ 'Element COMPANYID is undefined in SESSION.' and (not structKeyExists(session, "empid") or not structKeyExists(session, "companyid"))>
       		<cflocation url="index.cfm?event=login&AlertMessageID=2">
       	<cfelseif structKeyExists(arguments.Exception, "cause") and structKeyExists(arguments.Exception.cause, "Message") and arguments.Exception.cause.Message EQ 'Element COMPANYID is undefined in SESSION.' and structKeyExists(session, "empid") and  not len(session.empid)>
       		<cflocation url="index.cfm?event=login&AlertMessageID=2">
       	<cfelseif structKeyExists(arguments.Exception, "cause") and structKeyExists(arguments.Exception.cause, "Message") and arguments.Exception.cause.Message EQ 'Element PASSPORT.PASSWORDEXPIRED is undefined in SESSION.' and structKeyExists(url, "event") and url.event EQ 'login:process'>
       		<cflocation url="index.cfm?event=login&AlertMessageID=2">
       	<cfelseif structKeyExists(arguments.Exception, "cause") and structKeyExists(arguments.Exception.cause, "Message") and arguments.Exception.cause.Message EQ 'Element EMPID is undefined in SESSION.' and structKeyExists(session, "CompanyID") and len(session.CompanyID)>
       		<cflocation url="index.cfm?event=login&AlertMessageID=2">
       	</cfif>
		...
		<cfif isDefined("cgi.HTTP_REFERER")>
       	...
       	<cfelse>
       		<cfoutput>
       			...
       			<cfinclude template="ErrorReport.cfm"/>
       		</cfoutput>
       		<cfif findNoCase("/CarrierOnBoard/", cgi.script_name)>
       			<cffile action="append" file="#ExpandPath("../logs")#/Application.Log.txt" output="#dateTimeFormat(now())##chr(10)##(companycode)##arguments.Exception.cause.Detail##arguments.Exception.cause.Message##chr(10)#[URLData]:#SerializeJSON(url)##chr(10)#[FormData]:#SerializeJSON(form)##chr(10)#[SessionData]:#SerializeJSON(session)##trim(template)##sourcepage#">
       		<cfelse>
	       		<cffile action="append" file="#ExpandPath("../webroot/logs")#/Application.Log.txt" output="#dateTimeFormat(now())##chr(10)##(companycode)##arguments.Exception.cause.Detail##arguments.Exception.cause.Message##chr(10)#[URLData]:#SerializeJSON(url)##chr(10)#[FormData]:#SerializeJSON(form)##chr(10)#[SessionData]:#SerializeJSON(session)##trim(template)##sourcepage#">
	       	</cfif>
       		<cfquery name="qInsLog" datasource="LoadManagerAdmin">
       			INSERT INTO LMErrorLog (CompanyCode,Detail,Status,FormData,URLData,SessionData,Template,SourcePage)
       			VALUES (
       				<cfif structKeyExists(session, "usercompanycode")>
       				...
       				<cfif isDefined("session")>
       					,<cfqueryparam value="#SerializeJSON(session)#" cfsqltype="cf_sql_varchar">
       				<cfelse>
       					,NULL
       				</cfif>
       		...
       		<cfmail from='"support@loadmanager.com" <support@loadmanager.com>' subject="#left((stripHTMLapp(trim(arguments.Exception.cause.Detail&arguments.Exception.cause.Message))),100)#" to="support@loadmanager.com" type="html" server="smtpout.secureserver.net" username="support@loadmanager.com" password="Wsi2008!" port="465" usessl="1" usetls="0">
       			<cfoutput>
    ...
    <cffunction name="stripHTMLapp" access="public" returntype="string">
