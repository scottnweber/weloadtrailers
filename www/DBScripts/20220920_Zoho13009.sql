IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'SystemConfig' and column_name = 'ShowMaxCarrRateInLoadScreen')
BEGIN
	ALTER TABLE SystemConfig ADD ShowMaxCarrRateInLoadScreen bit not null default 0;
END


INSERT INTO SecurityParameter (ID,Category,Parameter,Description,Type) VALUES (NEWID(),'Settings','ShowMaxCarrRateInLoadScreen','This allows the user to set a "Maximum Carrier Rate" for the carriers flat rate, miles rate and accessorial rate, other users cannot increase it.','Load')


IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'Loads' and column_name = 'LimitCarrierRate')
BEGIN
	ALTER TABLE Loads ADD LimitCarrierRate bit not null default 0;
END

IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'Loads' and column_name = 'MaxCarrierRate')
BEGIN
	ALTER TABLE Loads ADD MaxCarrierRate float;
END

UPDATE Roles SET userRights = userRights  + ',ShowMaxCarrRateInLoadScreen' WHERE RoleValue ='Administrator' AND userRights   NOT LIKE '%ShowMaxCarrRateInLoadScreen%'


GO
/****** Object:  StoredProcedure [dbo].[USP_UpdateLoad]    Script Date: 19-09-2022 11:12:55 ******/
SET ANSI_NULLS ON
GO
...
,@CustomerPaid bit
,@CarrierPaid bit
,@LimitCarrierRate bit
,@MaxCarrierRate float
)
as
begin
...
-- return generated load ID
	...
	LimitCarrierRate=					@LimitCarrierRate,
...
GO
/****** Object:  StoredProcedure [dbo].[USP_GetLoadDetails]    Script Date: 19-09-2022 11:18:00 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[USP_InsertLoad]    Script Date: 19-09-2022 11:22:46 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
