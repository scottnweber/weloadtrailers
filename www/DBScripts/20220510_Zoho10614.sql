IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'BillFromCompanies' and column_name = 'Contact')
BEGIN
	ALTER TABLE BillFromCompanies ADD Contact varchar(150);
END
GO
IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'Companies' and column_name = 'MC')
BEGIN
	ALTER TABLE Companies ADD MC varchar(150);
END
GO
IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'Systemconfig' and column_name = 'MobileAppSwitchConfirmation')
BEGIN
	ALTER TABLE Systemconfig ADD MobileAppSwitchConfirmation BIT NOT NULL DEFAULT(1);
END
GO
/****** Object:  View [dbo].[vwCustomerInvoiceReportSub]    Script Date: 10-05-2022 09:40:56 ******/
SET ANSI_NULLS ON
GO
...
SELECT        TOP (100) PERCENT dbo.Loads.LoadID, CASE WHEN Isnull(dbo.Loads.invoiceNumber, 0) = 0 THEN dbo.Loads.LoadNumber ELSE dbo.Loads.invoiceNumber END AS LoadNumber, 
						 ...
						 CASE dbo.Factorings.PrintOnInvoice WHEN 1 THEN REPLACE(dbo.Factorings.InvoiceRemitInformation,'&nbsp;',' ') ELSE NULL END AS InvoiceRemitInformation,dbo.SystemConfig.ShowCustomerTermsOnInvoice,REPLACE(REPLACE(dbo.LoadStatusTypes.Reports,'RATE ','Customer '),'CANCELLED','INVOICE ') AS ReportTitle
...
/****** Object:  StoredProcedure [dbo].[USP_GetCarrierReport]    Script Date: 10-05-2022 11:27:21 ******/
SET ANSI_NULLS ON
GO
