...
/****** Object:  StoredProcedure [dbo].[USP_GetLateStopDetails]    Script Date: 22-03-2022 10:11:47 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER proc [dbo].[USP_GetLateStopDetails]
(
@LoadID varchar(max)
)
AS
BEGIN
	DECLARE @TotalLoads int,@TotalStops int,@LateStops int;
	SET @TotalLoads = (SELECT COUNT(ID) FROM dbo.fn_split( @LoadID, ','))
	SET @TotalStops = (SELECT COUNT(LoadStopID) FROM LoadStops WHERE LoadID IN (SELECT Value FROM dbo.fn_split( @LoadID, ',')))
	SET @LateStops = (SELECT COUNT(LoadStopID) FROM LoadStops WHERE LoadID IN (SELECT Value FROM dbo.fn_split( @LoadID, ',')) AND LateStop =1)

	SELECT @TotalLoads AS TotalLoads, @TotalStops AS TotalStops, @LateStops AS LateStops, CASE WHEN @TotalStops = 0 THEN 0 ELSE ((@LateStops*100)/@TotalStops) END AS LateStopPerc
