...
/****** Object:  View [dbo].[vwCustomerInvoiceReportSub]    Script Date: 13-06-2022 12:14:54 ******/
SET ANSI_NULLS ON
GO
...
SELECT        TOP (100) PERCENT dbo.Loads.LoadID, CASE WHEN Isnull(dbo.Loads.invoiceNumber, 0) = 0 THEN dbo.Loads.LoadNumber ELSE dbo.Loads.invoiceNumber END AS LoadNumber, 
						 ...
						 CASE dbo.Factorings.PrintOnInvoice WHEN 1 THEN REPLACE(REPLACE(REPLACE(dbo.Factorings.InvoiceRemitInformation,'&oacute;','ó'),'&bull;','&#8226;'),'&nbsp;',' ') ELSE NULL END AS InvoiceRemitInformation,dbo.SystemConfig.ShowCustomerTermsOnInvoice,REPLACE(REPLACE(dbo.LoadStatusTypes.Reports,'RATE ','Customer '),'CANCELLED','INVOICE ') AS ReportTitle
