IF(SELECT COUNT(ID) FROM SecurityParameter WHERE parameter = 'showAllOfficeLoads') = 0
BEGIN
INSERT INTO [SecurityParameter] 
([Category],[Parameter],[Description],[CreatedBy],[ModifiedBy],[Type]) 
VALUES ('Edit','showAllOfficeLoads','User is allowed to view all office loads.','Scott','Scott','Load')
END


GO

/****** Object:  StoredProcedure [dbo].[USP_GetLoadList]    Script Date: 14-06-2022 18:08:15 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	ELSE
		BEGIN
			...
			BEGIN
				SET @sql=@sql+' AND (SalesRepID='''+@empID+''' or DispatcherID = '''+@empID+''')'
			END

			IF(@currentusertype = 'dispatcher' AND ISNULL(LEN(@empID),0) <> 0 AND @showAllOfficeLoads = 0)
			BEGIN
				SET @sql=@sql+' AND DispatcherID = '''+@empID+''''
			END
...
/****** Object:  StoredProcedure [dbo].[USP_GetLoadListBETA]    Script Date: 15-06-2022 10:26:12 ******/
SET ANSI_NULLS ON
GO
