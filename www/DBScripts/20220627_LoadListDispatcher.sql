IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'Loads' and column_name = 'Dispatcher2Name')
BEGIN
	ALTER TABLE Loads ADD Dispatcher2Name varchar(150);
END
IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'Loads' and column_name = 'Dispatcher3Name')
BEGIN
	ALTER TABLE Loads ADD Dispatcher3Name varchar(150);
END
GO
UPDATE L SET 
L.Dispatcher2Name=D2.Name,L.Dispatcher3Name=D3.Name FROM Loads L
LEFT JOIN Employees D2 ON L.Dispatcher2 = D2.EmployeeID
LEFT JOIN Employees D3 ON L.Dispatcher3 = D3.EmployeeID


UPDATE SystemConfig SET LoadsColumns = LoadsColumns + ',Dispatcher 2',LoadsColumnsStatus=LoadsColumnsStatus+',0' WHERE LoadsColumns NOT LIKE '%Dispatcher 2%'

GO

/****** Object:  StoredProcedure [dbo].[USP_GetLoadList]    Script Date: 27-06-2022 10:56:26 ******/
SET ANSI_NULLS ON
GO
			...
			SET @sql=@sql+'	GROUP BY customerid) Offices ON Offices.CustomerID = l.CustomerID
					...
					LEFT OUTER JOIN (SELECT [Name] as empDispatch,[EmployeeID] FROM [Employees]) as Employees on Employees.EmployeeID =l.DispatcherID
						...
						LEFT JOIN Companies ON Companies.CompanyID = L.BillFromCompany
	      				where l.LoadID = l.LoadID AND loadStatus.IsActive = 1';
...
/****** Object:  StoredProcedure [dbo].[USP_GetLoadListBETA]    Script Date: 27-06-2022 12:47:46 ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROC [dbo].[USP_GetLoadListBETA]
(
...
BEGIN
	...
	ELSE
		BEGIN
			...
			IF(@currentusertype = 'sales representative' AND ISNULL(LEN(@empID),0) <> 0 AND @showAllOfficeLoads = 0)
			BEGIN
				SET @sql=@sql+' AND (SalesRepID='''+@empID+''' or SalesRep1='''+@empID+''' or SalesRep2='''+@empID+''' or SalesRep3='''+@empID+''' or L.DispatcherID = '''+@empID+''' or Dispatcher1 = '''+@empID+''' or Dispatcher2 = '''+@empID+''' or Dispatcher3 = '''+@empID+''')'
			END

			IF(@currentusertype = 'dispatcher' AND ISNULL(LEN(@empID),0) <> 0 AND @showAllOfficeLoads = 0)
			BEGIN
				SET @sql=@sql+' AND L.DispatcherID = '''+@empID+''' or Dispatcher1 = '''+@empID+''' or Dispatcher2 = '''+@empID+''' or Dispatcher3 = '''+@empID+''''
			END
...
/****** Object:  StoredProcedure [dbo].[USP_InsertLoad]    Script Date: 27-06-2022 13:15:39 ******/
SET ANSI_NULLS ON
GO
...
,@Dispatcher2
,@Dispatcher3
,(SELECT Name FROM Employees WHERE EmployeeID = @Dispatcher2)
,(SELECT Name FROM Employees WHERE EmployeeID = @Dispatcher3)
,@Dispatcher1Percentage
,@Dispatcher2Percentage
,@Dispatcher3Percentage
,@BillFromCompany
