IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'SystemConfig' and column_name = 'CustomerPaymentTerms')
BEGIN
	ALTER TABLE SystemConfig ADD CustomerPaymentTerms varchar(100);
END
GO

GO

/****** Object:  StoredProcedure [dbo].[USP_ImportCustomerViaCSV]    Script Date: 01-07-2022 13:06:54 ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN
		...
		BEGIN
			...
			ELSE
				...
				END
		END
		IF(LEN(TRIM(@BestOpp)))=0
		BEGIN
			SET @BestOpp = (SELECT CustomerPaymentTerms FROM SystemConfig WHERE CompanyID = @CompanyID)
		END
