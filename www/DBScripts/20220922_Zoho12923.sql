IF NOT EXISTS (SELECT column_name FROM INFORMATION_SCHEMA.columns where table_name = 'SystemConfig' and column_name = 'NonCommissionable')
BEGIN
	ALTER TABLE SystemConfig ADD NonCommissionable bit not null default 0;
END
...
/****** Object:  View [dbo].[vwSalesRepCommission]    Script Date: 22-09-2022 13:53:14 ******/
SET ANSI_NULLS ON
GO
...
SELECT        TOP (100) PERCENT dbo.Companies.CompanyName,dbo.Companies.CompanyID, dbo.Employees.Name AS Dispatcher, ISNULL(NULLIF (Employees_1.Name, N''), N'(BLANK)') AS SalesAgent1, dbo.Loads.LoadNumber, 
                         ...
                         dbo.LoadStops.City + ', ' + dbo.LoadStops.StateCode AS FirstStop, dbo.vwCarrierConfirmationReportFinalDestination.City + ', ' + dbo.vwCarrierConfirmationReportFinalDestination.StateCode AS LastStop,dbo.Loads.TotalDirectCost, 
