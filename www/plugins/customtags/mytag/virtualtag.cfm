   ...
   <cfif thisTag.ExecutionMode is 'start'>
    <cfif isDefined("attributes.officeloc")>
        ...
        <cfquery name="qrygetoffices" datasource="#application.dsn#">
            select * from Offices where isActive = 'True' and CompanyID = '#session.companyid#'
            ORDER BY Location
        </cfquery> 
        
        </cfsilent>   
	    <select name="OfficeID1" <cfif isDefined("attributes.tabindex")> tabindex="#attributes.tabindex#" </cfif> multiple style="height: 100px;">
	    <cfloop query="qrygetoffices">
	        <option value="#qrygetoffices.officeid#" <cfif listFind("#attributes.officeloc#", qrygetoffices.officeid)
          OR (isDefined("attributes.customerID") and len(trim(attributes.customerID)) eq 1 AND qrygetoffices.recordcount eq 1)
          > selected="selected" </cfif> >#qrygetoffices.location#</option>
	    </cfloop>
	    </select>           
  </cfif> 
      ...
      <cfquery name="qrygetcompanies" datasource="#application.dsn#">
         select * from Companies                
          <cfif isdefined('attributes.CompanyID') and len(attributes.CompanyID)>
            where CompanyID  = '#attributes.CompanyID#'
          </cfif>
