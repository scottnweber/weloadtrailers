...
<cfoutput>
	...
	<cfif structkeyexists(url,"equipmentMaintId") and structkeyexists(url,"equipmentid") and len(trim(url.equipmentMaintId)) and len(trim(url.equipmentid)) gt 1>
		...
		<div class="search-panel">
			<div class="delbutton">
				<a href="index.cfm?event=addDriverEquipment&equipmentid=#editid#&equipmentMaintId=#editEquipmentmMaintId#&deleteMaintItem=1&#session.URLToken#&IsCarrier=#url.IsCarrier#" onclick="return confirm('Are you sure to delete it ?');">
				Delete </a>
			</div>
		</div>	
	...
	<div class="white-con-area">
		...
		<div class="white-mid">
			<cfform name="frmPopUpMaintenance" action="index.cfm?event=addNewMaintenance:process&#session.URLToken#&IsCarrier=#url.IsCarrier#" method="post" enctype="multipart/form-data">
				<div class="form-con">
					<fieldset>
						<cfif not len(trim(description)) or not len(trim(editEquipmentmMaintId))>
							<label>Description</label>
							<select onchange="getmaintenancesetUpValues('#dsn#','#session.companyid#');" tabindex="1" style="width:194px;" id="description" name="description">
						...
						<label>Next Date</label>
							<input name="nextDate" type="datefield" id="Date" style="width:75px;" onblur="checkDateFormatAll(this);" value="#Dateformat(Nextdate,'mm/dd/yyyy')#" tabindex="4" class="datefield">
						<div class="clear"></div>
						...
						<input  type="button" onclick="document.location.href='index.cfm?event=addNewMaintenance:process&equipmentid=#editid##session.URLToken#&IsCarrier=#url.IsCarrier#'"  name="back" class="bttn" value="Back" style="width:70px;margin-top: 5px;" tabindex="7" />
		...
		<cfif structkeyexists(url,"equipmentMaintId") and  structkeyexists(url,"equipmentid") and len(trim(url.equipmentMaintId)) and len(trim(url.equipmentid)) gt 1>
			<div class="addbutton"  style="cursor: pointer;padding-bottom: 8px;"><a href="index.cfm?event=addNewMaintenanceTransaction&equipmentid=#request.qEquipments.EquipmentID#&equipmentMaintId=#request.qEquipmentMaint.EquipMainID#&pageDirection=1&#session.URLToken#&IsCarrier=#url.IsCarrier#">Add Trans</a></div>
