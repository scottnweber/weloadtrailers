...
<cfif request.qGetSystemSetupOptions.CustomerLNI EQ 1>
		...
		<cfif isdefined('variables.responsestatus')>
			<cfif not ArrayIsEmpty(variables.responsestatus.content)>
				...
				<cfif  not cfhttp.FileContent contains 'No Data Available'>
					...
					<cfset insCarrier = left(trim(mid(myStr1,(getUSDOTFrom+17),USDOTLen)),100)>
					...
					<cfif findnocase('<TD headers="34 policy_surety ">',myStr1,1) neq 0>
						...
						<cfset insPolicy = left(trim(mid(myStr1,(getUSDOTFrom+34),USDOTLen)),50)>
					<cfelse>
...
<cfelse>
	<cfif isdefined('url.mcNo') and len(url.mcNo) gt 1>
		<cfset Number=#url.mcNo#><!--- MC number is said to be deprecated. Doing this for showing NOT FOUND message for now.--->
		<cfset isDot = 0>
	<cfelseif isdefined('url.DOTNumber') and len(url.DOTNumber) gt 1>
		<cfset Number=#url.DOTNumber#> 
		<cfset isDot = 1>
	</cfif>
		
	<cfinvoke component="#variables.objCarrierGateway#" method="getCarrierFromFmcsaDB" NUMBER =#Number# isDOT=#isDot# dsn=#Application.FMCSAdsn# returnvariable="request.qCarrierFromFmcsaDB" />

	<cfif request.qCarrierFromFmcsaDB.recordcount>
		<cfset DOTNumber = request.qCarrierFromFmcsaDB.DOT_NUMBER>
		<cfset CarrierName = request.qCarrierFromFmcsaDB.LEGAL_NAME>
		<cfset Address = request.qCarrierFromFmcsaDB.PHY_STREET>
		<cfset City = request.qCarrierFromFmcsaDB.PHY_CITY>
		<cfset State11 = request.qCarrierFromFmcsaDB.PHY_STATE>
		<cfset Zipcode = request.qCarrierFromFmcsaDB.PHY_ZIP>
		<cfset Country1 = request.qCarrierFromFmcsaDB.PHY_COUNTRY>
		<cfset businessPhone =  request.qCarrierFromFmcsaDB.TELEPHONE>	
		<cfset Fax = REReplace(request.qCarrierFromFmcsaDB.FAX, "[^0-9]", "", "ALL")>
		<cfset Fax = "#left(Fax,3)#-#mid(Fax,4,3)#-#right(Fax,4)#">
		<cfset Email = request.qCarrierFromFmcsaDB.EMAIL_ADDRESS>	
	<cfelse>
		<cfif structkeyExists(url,"mcNo") >
			<cfset message="Data not available. Please check the mc## and try again.">
		</cfif>
