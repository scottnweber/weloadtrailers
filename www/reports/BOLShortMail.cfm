...
<cfif IsDefined("form.send")>
	<cfif form.MailTo is not "" AND form.MailFrom is not "" AND form.Subject is not "">
        ...
        <cfquery name="BOLReportShort" datasource="#application.dsn#">
								...
								dbo.Employees ON L.DispatcherID = dbo.Employees.EmployeeID LEFT OUTER JOIN
									...
									<cfif qGetSystemconfig.UseNonFeeAccOnBOL EQ 1>
										...
										(SELECT ISNULL(dbo.LoadStopCommodities.SrNo, '') AS SrNo, ISNULL(dbo.LoadStopCommodities.Qty, 0) AS Qty, ISNULL(dbo.LoadStopCommodities.Description, '') AS Description, 
											...
											<cfqueryparam value="#url.loadid#" cfsqltype="cf_sql_varchar"> AS LoadStopIDBOL,
		...
		ORDER BY l.LoadNumber
        	...
        	<cfoutput>
                <!--- <cfreport name="genPDF" format="PDF" template="ShortBOLReport.cfr" query="#BOLReportShort#"> 
                ...
                <cfif application.dsn neq "LoadManagerLive"> --->
                    ...
                    <cfif len(trim(qBOLReportShort.CompanyCode)) and directoryExists(expandPath("../reports/#trim(qBOLReportShort.CompanyCode)#")) and fileExists(expandPath("../reports/#trim(qBOLReportShort.CompanyCode)#/BOLReportShort.cfm"))>
                        <cfinclude template="#trim(qCustomerReport.CompanyCode)#/BOLReportShort.cfm">
                    <cfelse>
                        <cfinclude template="BOLReportShort.cfm">
                        <cfset genPDF = BOLReportShort>
                        <cfset BOLReportShort = qBOLReportShort>
                    </cfif>
                <!--- </cfif> --->
			<cftry>
				<cfif request.qGetCompanyInformation.ccOnEmails EQ true>
					...
					#form.body#
						<cfmailparam
							file="#BOLReportShort.LoadNumber#.BOLReportShort.pdf"
...
<p> 
<form action = "index.cfm?event=loadMail&type=BOLShort&loadid=#loadID#&#session.URLToken#" method="POST">
	<div class="white-mid" style=" border-top: 5px solid rgb(130, 187, 239);width: 100%;">
		<div class="form-con" style="width:auto;">
			...
			<fieldset>
			<div style="color:##000000;font-size:14px;font-weight:bold;margin-bottom:20px;margin-top:10px;">Short BOL Report Mail</div>
			<div class="clear"></div>
