<cfoutput>
    <cftry>
        ...
        <cfquery name="qIncomeStatement" datasource="#local.dsn#">
            ...
            FROM [LMA General Ledger Balance Sheet Setup] ISS 
            ...
            <cfif structKeyExists(url, "DeptFilter") AND url.DeptFilter>
                <cfif LEN(TRIM(url.FromDept)) OR LEN(TRIM(url.ToDept))>
                    ...
                    <cfif LEN(TRIM(url.FromDept))>
                        AND RIGHT(GL_ACCT,4) >= <cfqueryparam value="#TRIM(url.FromDept)#" cfsqltype="cf_sql_varchar">
                    </cfif>
                    <cfif LEN(TRIM(url.ToDept))>
                        AND RIGHT(GL_ACCT,4) <= <cfqueryparam value="#TRIM(url.ToDept)#" cfsqltype="cf_sql_varchar">
                    </cfif>
                </cfif>
            </cfif>
    	...
    	<cfset fileName = "Balance Sheet Date-#DateFormat(Now(),'mm.dd.yyyy')#.pdf">
    	<cfheader name="Content-Disposition" value="inline; filename=#fileName#">
    	<cfdocument format="PDF" name="Balancesheet"  margintop="2" orientation="landscape">
    	        ...
    	        <BODY style="font-family: 'Arial';">
                    <cfdocumentitem type="header">
                        <table width="100%" style="font-family: 'Arial';border-bottom: solid 6px ##C0C0C0;border-top: solid 3px ##C0C0C0;">
                            ...
                            <tr>
                                <td width="50%">
                                    <h2><i>Balance Sheet - Monthly</i></h2>
                                </td>
                                    ...
                                    <cfloop group="MainTitle">
                                        ...
                                        <cfelse>
                                            <cfloop group="GL_ACCT">
                                                <cfset CurrentMonthPerc = (CurrentMonth/CurrentMonthTotal)*100>
                                                <cfset CurrentYTDPerc = (CurrentYTD/CurrentYTDTotal)*100>
                                                <cfif PriorYearMonthTotal NEQ 0>
                                                    <cfset PriorYearMonthPerc = (PriorYearMonth/PriorYearMonthTotal)*100>
                                                <cfelse>
                                                    <cfset PriorYearMonthPerc = 0>
                                                </cfif>
                                                <cfif PriorYearYTDTotal NEQ 0>
                                                    <cfset PriorYearYTDPerc = (PriorYearYTD/PriorYearYTDTotal)*100>
                                                <cfelse>
    	...
    	<cfcontent variable="#toBinary(Balancesheet)#" type="application/pdf" /> 	
        <cfcatch>
            <cfdump var="#cfcatch#"><cfabort>
            <cfset fileName = "Balance Sheet Date-#DateFormat(Now(),'mm.dd.yyyy')#.pdf">
            <cfheader name="Content-Disposition" value="inline; filename=#fileName#">
            <cfdocument format="PDF" name="Balancesheet" margintop="1.5">
                <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
                <HTML xmlns="http://www.w3.org/1999/xhtml">
                    <HEAD>
                        <meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
