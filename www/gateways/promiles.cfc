<cfcomponent output="false">
	...
	<cffunction name="insertLoadIFTAMiles" access="public" returntype="void" >
		...
		<cfquery name="qryInsertLoadIFTAMiles" datasource="#arguments.datasource#">
			...
			VALUES
			(
				<cfqueryparam value="#arguments.loadNum#" cfsqltype="cf_sql_bigint">,
	...
	<cffunction name="deleteLoadIFTAMiles" access="public" returntype="void" >
		...
		<cfquery name="qryDeleteLoadIFTAMiles" datasource="#arguments.datasource#">
			delete from 
				LoadIFTAMiles 
			where 
				loadNumber = <cfqueryparam value="#arguments.loadNum#" cfsqltype="cf_sql_bigint">
		</cfquery>
	</cffunction>
	...
	<cffunction name="promilesCalculation" access="public" returntype="any">
		...
		<cfif not isDefined("Application")>
			...
			<cfset Application.dsn=arguments.frmstruct.dsn>
			<cfset Session.empid=arguments.frmstruct.empid>
			<cfset allinfoTemp = structNew()>
			<cfset allinfoTemp.LoadID = arguments.frmstruct.loadid>
			<cfset session.allinfo = allinfoTemp>
		</cfif>
		...
		<cfquery dbtype="query" name="qryGetStatusTextNumbersExtactMain">  
			select statusNumberText from qryGetStatusTextNumbersSet where statusNumberText between 5 and 8.9
		</cfquery>
		...
		<cfquery dbtype="query" name="qryGetStatusTextNumbersExtact">  
			select statusNumber from qryGetStatusTextNumbers where statusNumber between 5 and 8.9
		</cfquery>
		...
		<cfif structKeyExists(session,"empid")>
			<cfquery name="getProMileDetails" datasource="#Application.dsn#">
				select PCMilerUsername,PCMilerPassword,PCMilerCompanyCode,
				(select CompanyCode from companies where companyid = <cfqueryparam value="#session.companyid#" cfsqltype="cf_sql_varchar">) as companyCode
				from SystemConfig 
		...
		<cfif structKeyExists(session, "allinfo") and structKeyExists(session.allinfo, "LoadID") and len(session.allinfo.LoadID)>
			<cfquery name="qGetProMilesUnitID" datasource="#Application.dsn#">
					SELECT 	l.LoadNumber, 
							l.LoadID, 
							Eq.ProMilesUnitID, 
							ISNULL(Eq.IFTA,0) AS IFTAinclude,
							Ls.LoadType, 
							Ls.StopNo
					FROM Loads l
		...
		<cfif isDefined("qGetProMilesUnitID") and qGetProMilesUnitID.recordcount>
			<cfset local.ProMilesUnitID = qGetProMilesUnitID.ProMilesUnitID>
		</cfif>
		<cfif isdefined("arguments.frmstruct.IFTAinclude")>
			<cfset local.IFTAinclude = arguments.frmstruct.IFTAinclude>
		<cfelseif isDefined("qGetProMilesUnitID") and qGetProMilesUnitID.recordcount>
			<cfset local.IFTAinclude = qGetProMilesUnitID.IFTAinclude>
		</cfif>
					...
					<soap:Body>
						<RunTrip xmlns="http://promiles.com/">
							...
							<Trip>
								...
								<TripLegs>
									<cfif isdefined("arguments.frmstruct.dhSHIPPERCITY") AND len(trim(arguments.frmstruct.dhSHIPPERCITY)) and len(trim(arguments.frmstruct.dhSHIPPERSTATE)) and len(trim(arguments.frmstruct.dhSHIPPERZIPCODE))>
										<TripLeg>
											<LocationText>#arguments.frmstruct.dhSHIPPERCITY#, #arguments.frmstruct.dhSHIPPERSTATE# #arguments.frmstruct.dhSHIPPERZIPCODE#</LocationText>
										</TripLeg>
									</cfif>
						...
						<cfif IsDefined('xmlToStruct.Body.RunTripResponse.RunTripResult.Results')>
							...
							<cfif ListFindNoCase(variables.statuslist,qryGetStatusTextNumbersExtactMain.statusNumberText)>
								<cfquery name="qryGetLoadid" datasource="#application.dsn#">
									select loadid from loads where loadnumber=<cfqueryparam value="#arguments.frmstruct.loadnumber#" cfsqltype="cf_sql_bigint">
								</cfquery>
		...
		<cfif qIFTATaxSummary.recordCount>			
			...
			<cfelse>
				<cfquery name="qDeleteTaxSummaryRow" datasource="#application.dsn#">
					DELETE FROM IFTATaxSummary WHERE loadnumber = <cfqueryparam value="#arguments.loadNum#" cfsqltype="cf_sql_bigint">
				</cfquery>
			</cfif>
	...
	<cffunction name="getIFTATaxSummary" access="public" returntype="Query" output="false" hint="get all tax summary details of a loadID">
		...
		<cfquery name="qGetIFTATaxSummary" datasource="#arguments.datasource#">
			SELECT * FROM IFTATaxSummary WHERE loadNumber = <cfqueryparam value="#arguments.loadNum#" cfsqltype="cf_sql_bigint">
		</cfquery>
	...
	<cffunction name="DeleteTaxSummaryRows" access="public" returntype="void" output="false" hint="Deletes all tax summary details of a loadID">
		...
		<cfquery name="qDeleteTaxSummaryRow" datasource="#arguments.datasource#">
			DELETE FROM IFTATaxSummaryRow WHERE loadnumber = <cfqueryparam value="#arguments.loadNum#" cfsqltype="cf_sql_bigint">
		</cfquery>
	</cffunction>
